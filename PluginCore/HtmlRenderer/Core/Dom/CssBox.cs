...
namespace TheArtOfDev.HtmlRenderer.Core.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        public bool IsBlock
        {
            get { return Display == CssConstants.Block || Display == CssConstants.Table; }
        }
        ...
        protected virtual void PerformLayoutImp(RGraphics g)
        {
            ...
            if (IsBlock || Display == CssConstants.ListItem || Display == CssConstants.Table || Display == CssConstants.InlineTable || Display == CssConstants.TableCell)
            {
                ...
                else
                {
                    ...
                    if (DomUtils.ContainsInlinesOnly(this) && !this.IsBrElement)
                    {
        ...
        protected double MarginTopCollapse(CssBoxProperties prevSibling)
        {
            ...
            if (prevSibling != null)
            {
                if (this.Height == "auto" && this.ActualHeight == 0 && this.ActualPaddingBottom == 0 &&
                    this.ActualPaddingTop == 0 && this.ActualBorderTopWidth == 0 && this.ActualBorderBottomWidth == 0 &&
                    this.IsSpaceOrEmpty)
                {
                    value = 0;
                }
            ...
            else if (_htmltag != null && _htmltag.Name == "p")
            {
                value = 0;
            }
        ...
        {
            double margin = 0;
            var lastChildBox = _boxes[_boxes.Count - 1];
            if (ParentBox != null && _parentBox.ActualMarginBottom < 0.1)
            {
                var lastChildBottomMargin = lastChildBox.ActualMarginBottom;

                if (Height == "auto")
                {
                    if (lastChildBox.HtmlTag != null && lastChildBox.HtmlTag.Name == "p")
                    {
                        margin = 0;
                    }
                    else
        ...
        protected override RColor GetActualColor(string colorStr)
        {
            ...
            var foreColor = HtmlContainer.CssParser.ParseColor(colorParts[0]);
            if (colorParts.Length > 1)
            {
                var parent = this.ParentBox;
                var parentColor = RColor.Empty;
                while ((parent = parent.ParentBox) != null)
                {
                    if (!parent.ActualBackgroundColor.IsEmpty && parent.ActualBackgroundColor.A > 0)
                    {
                        parentColor = parent.ActualBackgroundColor;
                        break;
                    }
                }
                foreColor = Mix(parentColor, foreColor, colorParts[1]);
            }
            return foreColor;
        }
        ...
        public static RColor Mix(RColor back, RColor fore, string mode)
        {
            if (back.IsEmpty)
                return fore;

            float backR = back.R / 255.0f;
            float backG = back.G / 255.0f;
            float backB = back.B / 255.0f;

            float foreA = fore.A / 255.0f;
            float foreR = fore.R / 255.0f;
            float foreG = fore.G / 255.0f;
            float foreB = fore.B / 255.0f;

            float outR = MixChannel(backR, foreR, mode);
            float outG = MixChannel(backG, foreG, mode);
            float outB = MixChannel(backB, foreB, mode);
            ...
            return RColor.FromArgb(255, (int)(255 * outR), (int)(255.0f * outG), (int)(255.0f * outB));
        }

        public static float MixChannel(float back, float fore, string mode)
        {
            _InitStatics();

            if (!_colorChannelMixers.ContainsKey(mode))
                mode = "NORMAL";
        ...
        private static Dictionary<string, Func<float, float, float>> _colorChannelMixers;
        ...
        private static void _InitStatics()
        {
            ...
            _colorChannelMixers = new Dictionary<string, Func<float, float, float>>(StringComparer.OrdinalIgnoreCase);
            _colorChannelMixers["NORMAL"] = _channelMixer_NORMAL;
            _colorChannelMixers["ADD"] = _channelMixer_ADD;
            _colorChannelMixers["SUBTRACT"] = _channelMixer_SUBTRACT;
            _colorChannelMixers["MULTIPLY"] = _channelMixer_MULTIPLY;
            _colorChannelMixers["DIVIDE"] = _channelMixer_DIVIDE;
            _colorChannelMixers["DIFFERENCE"] = _channelMixer_DIFFERENCE;
            _colorChannelMixers["EXCLUSION"] = _channelMixer_EXCLUSION;
            _colorChannelMixers["OVERLAY"] = _channelMixer_OVERLAY;
            _colorChannelMixers["HARDLIGHT"] = _channelMixer_HARDLIGHT;
